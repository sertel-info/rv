<div class="form-group row">
	<div class='col-md-4 '>
		<center>
				...
				{{Form::checkbox('atend_automatico', 1, (Boolean)$linha->facilidades->atend_automatico, ['class'=>'form-control'])}}
		</center>
	</div>
	...
	<div class='col-md-8 collapse'>
			...
			<div class='col-md-6'>
				...
				<select name="atend_automatico_tipo" class='form-control'>
					...
					<option value="grupo" {{isset($linha) && $linha->facilidades->atend_automatico_tipo == "grupo" ? 'selected' : ''}} > Grupo </option>
					<option value="fila" {{isset($linha) && $linha->facilidades->atend_automatico_tipo == "fila" ? 'selected' : ''}} > Fila </option>
					<option value="ura" {{isset($linha) && $linha->facilidades->atend_automatico_tipo == "ura" ? 'selected' : ''}} > Ura </option>
				</select>
...
$(function(){
	...
	$("select[name=atend_automatico_tipo]").on("change", function(ev){
		...
		ev.preventDefault();
   		...
   		var el = $(this);
		var alvo = el.parent().parent().find('select[name=atend_automatico_destino]');
